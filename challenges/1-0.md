...
The deploy key generated in [Required resources](#get-required-resources) needs to be added to the manifest repo voting-app-manifest (created in [Challenge 0](0.md)).
...
Add the [azure-vote.yaml](../azure-vote.yaml) resource manifest for the voting application to the voting-app-manifest repository (created in [Challenge 0](0.md)).

## SSH key values
### Get node key value
In the [Required resources](#get-required-resources) step, we created the node key. Check the location of the file in infra-values.txt. It should look similar to this: ### 
